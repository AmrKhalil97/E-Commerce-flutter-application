            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  height: 100,
                  decoration: BoxDecoration(
                    color: Color(0xff800080).withOpacity(0.08),
                  ...
                  child: DottedBorder(
                      ...
                      dashPattern: [10, 5],
                      color: Color(0xff800080).withOpacity(0.6),
                      strokeWidth: 2,
                      child:
                      Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                Text('Current Vlock size: ${widget.count+1}',style: TextStyle(
                                  fontSize: 16,
                                  fontFamily: 'Raleway',
                            ...
                            Row(
                              children: [
                                Row(
                                  children: [
                                    Text('Your Price: €${widget.currentPrice}',style: TextStyle(
                                        fontSize: 16,
                                        fontFamily: 'Raleway',
                                        fontWeight: FontWeight.bold,
...
while(shippingTo !='' && paymentMethod !='' && deliveryPeriod != ''){
    ...
    int x=widget.currentVlockSize+1;
    var batch =await FirebaseFirestore.instance.batch();
    //Updates the field value, using post as document reference
    batch.update(post, { 'currentSize': x});
    batch.update(post, { 'count': widget.count+1});
    batch.update(post, { 'price': (widget.currentPrice-(widget.currentPrice*0.05)).roundToDouble()});
    batch.commit();

    print('updated');print(widget.id);
setState(() {
  widget.currentPrice=(widget.currentPrice-(widget.currentPrice*0.05)).roundToDouble();
  widget.currentVlockSize=x;
});
    final docPost=await FirebaseFirestore.instance.collection('orders');
    orderModel_class model= orderModel_class(
      seller: widget.seller,
      CatalogPrice: widget.catalogPrice,
      endPointPrice: widget.endPointPrice,
      currentVlockPrice: widget.currentPrice,
      startPointPrice:widget.startPointPrice ,
      productName: widget.productName,
      deliveryPeriod: deliveryPeriod,
      paymentMethod: paymentMethod,
      shippingTo: shippingTo,
      actualVlockSize:widget.actualVlockSize,
      currentVlockSize:widget.currentVlockSize,
      buyer: uId!
  ); print('8');
  docPost.add(model.toMap()); print('11');
    ...
    notificationsModel_class model2= notificationsModel_class(
        title: "${widget.count+1} users have joined ${widget.publisherName.split(' ')[0].trim()}'s Vlock",
        userImage: widget.publisherAvatar
    );
  ...
  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
    content: Text("offer successfully created"),
  ));
