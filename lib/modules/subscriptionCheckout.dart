...
class subscriptionCheckout extends StatefulWidget {
  ...
  @override
  State<subscriptionCheckout> createState() => _checkoutState();
  String publisher='';
  String productName='';
...
class _checkoutState extends State<subscriptionCheckout> {
                        ...
                        if (formKey.currentState!.validate()) {
                          ...
                          while(shippingTo !='' && paymentMethod !='' && deliveryPeriod != ''){
                            ...
                            if(widget.endPoint == true){
                              ...
                              int x=widget.actualVlockSize+1;
                              var batch =await FirebaseFirestore.instance.batch();
                              //Updates the field value, using post as document reference
                              batch.update(post, { 'size': x});
                              batch.commit();
                              setState(() {
                                widget.catalogPrice=(widget.catalogPrice-(widget.catalogPrice*0.05)).roundToDouble();
                                widget.actualVlockSize=x;
                              });
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                                content: Text("Successfully joined the vlock"),
                              ));
                              ...
                              //Updates the field value, using post as document reference
                              batch.update(post, { 'size': x});
                              batch.update(post, { 'vlockPrice': (widget.vlockPrice-(widget.vlockPrice*0.05)).roundToDouble()});
                              batch.commit();
                              setState(() {
                                widget.catalogPrice=(widget.catalogPrice-(widget.catalogPrice*0.05)).roundToDouble();
                                widget.actualVlockSize=x;
